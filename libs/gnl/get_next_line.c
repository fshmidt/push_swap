...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/10/09 16:40:15 by mbesan            #+#    #+#             */
/*   Updated: 2022/01/27 17:26:31 by mbesan           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	res = ft_reading(res, buf, fd);
	if (buf)
		free (buf);
	if (!res)
		return (NULL);
	output = ft_output(res, &indent);
	res = ft_res_rewrite(res, indent);
	return (output);
}
